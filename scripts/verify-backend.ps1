...
$r1 = Invoke-RestMethod -Method Get -Uri "http://localhost:$Port/faturamento/faturas?page=1&pageSize=5&sortBy=criadaEm&sortOrder=DESC" -Headers @{ Authorization = ("Bearer $token") }
...
$r2 = Invoke-RestMethod -Method Get -Uri "http://localhost:$Port/faturamento/faturas/paginadas?page=1&pageSize=5&sortBy=criadaEm&sortOrder=DESC" -Headers @{ Authorization = ("Bearer $token") }
