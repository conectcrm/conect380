...
```
Rota: POST /api/atendimento/webhooks/whatsapp/:empresaId
Status: ✅ FUNCIONANDO
    ...
    "entry": [{
      "changes": [{
        "value": {
          ...
          "contacts": [{ "profile": { "name": "Cliente Teste Fluxo" }, "wa_id": "5511999887766" }],
          "messages": [{ "from": "5511999887766", "text": { "body": "Olá" }, "type": "text" }]
        }
      }]
    }]
  }'
...
2. **Atualizar Meta Developer Console**:
  - Callback URL: `https://SEU-NGROK.ngrok.io/api/atendimento/webhooks/whatsapp/<ID_EMPRESA>`
   - Verify Token: `conectcrm_webhook_token_123`
  - Subscrever evento: `messages`
  - Confirmar envio do header `X-Hub-Signature-256`
