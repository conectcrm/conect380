# ============================================
# Script de Deploy Frontend - ConectCRM
# ============================================
# Automatiza o processo de build e deploy do frontend na AWS
# 
...
$IMAGE_NAME = "conectcrm-frontend:latest"
$CONTAINER_NAME = "conectcrm-frontend-prod"
$TAR_FILE = "frontend.tar"
$REACT_APP_API_URL = "http://56.124.63.239:3500"

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "🎨 Deploy Frontend - ConectCRM" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
if (-not $SkipBuild) {
  ...
  Write-Host "    Usando: Dockerfile.frontend-simple (build já pronto)" -ForegroundColor Gray
    
  $buildStart = Get-Date
    
  docker build -f .production/docker/Dockerfile.frontend-simple -t $IMAGE_NAME . 2>&1 | Out-Null
...
Write-Host "  Frontend: http://$AWS_IP:3000" -ForegroundColor White
Write-Host ""
Write-Host "Comandos úteis:" -ForegroundColor Cyan
Write-Host "  Logs: ssh -i `"$AWS_KEY`" $AWS_USER@$AWS_IP 'sudo docker logs -f $CONTAINER_NAME'" -ForegroundColor White
Write-Host "  Status: ssh -i `"$AWS_KEY`" $AWS_USER@$AWS_IP 'sudo docker ps | grep frontend'" -ForegroundColor White
Write-Host ""
