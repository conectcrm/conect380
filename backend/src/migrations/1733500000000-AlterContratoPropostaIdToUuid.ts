...
export class AlterContratoPropostaIdToUuid1733500000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const col = await queryRunner.query(`SELECT data_type FROM information_schema.columns WHERE table_name = 'contratos' AND column_name = 'propostaId'`);
    if (col?.[0]?.data_type === 'uuid') {
      return; // já aplicado
    }

    await queryRunner.query(`ALTER TABLE "contratos" RENAME COLUMN "propostaId" TO "propostaId_old"`);
    ...
    await queryRunner.query(`UPDATE "contratos" c SET "propostaId" = p.id FROM "propostas" p WHERE CAST(c."propostaId_old" AS text) = p.numero`);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`UPDATE "contratos" c SET "propostaId_old" = CAST(p.numero AS integer) FROM "propostas" p WHERE c."propostaId" = p.id AND p.numero ~ '^\\d+$'`);

    await queryRunner.query(`ALTER TABLE "contratos" DROP COLUMN "propostaId"`);
    await queryRunner.query(`ALTER TABLE "contratos" RENAME COLUMN "propostaId_old" TO "propostaId"`);
  }
}
