...
export class RefatorarTicketsConfiguraveis1735421400000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`CREATE INDEX idx_niveis_empresa_ordem ON niveis_atendimento(empresa_id, ordem)`);
    ...
    await queryRunner.query(`CREATE INDEX idx_status_empresa_nivel_ordem ON status_customizados(empresa_id, nivel_id, ordem)`);
    ...
    await queryRunner.query(`CREATE INDEX idx_tipos_empresa_ordem ON tipos_servico(empresa_id, ordem)`);
    ...
    const tiposServico = [
      { nome: 'Suporte', descricao: 'Suporte técnico geral', cor: '#8B5CF6', icone: 'HelpCircle', ordem: 1 },
      { nome: 'Técnica', descricao: 'Questões técnicas e troubleshooting', cor: '#3B82F6', icone: 'Wrench', ordem: 2 },
      { nome: 'Comercial', descricao: 'Questões comerciais e vendas', cor: '#10B981', icone: 'DollarSign', ordem: 3 },
      { nome: 'Financeira', descricao: 'Questões financeiras e pagamentos', cor: '#F59E0B', icone: 'CreditCard', ordem: 4 },
      { nome: 'Reclamação', descricao: 'Reclamações e insatisfações', cor: '#EF4444', icone: 'AlertTriangle', ordem: 5 },
      { nome: 'Solicitação de Melhoria', descricao: 'Sugestões de melhorias', cor: '#06B6D4', icone: 'Sparkles', ordem: 6 },
      { nome: 'Bug/Outros', descricao: 'Bugs reportados e outros assuntos', cor: '#EC4899', icone: 'Bug', ordem: 7 },
    ];
