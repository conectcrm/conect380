...
export class AddStatusToEmpresas1763916805124 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const historicoPlanosExists = await queryRunner.hasTable('historico_planos');
    const modulosEmpresasExists = await queryRunner.hasTable('modulos_empresas');
    const transacoesGatewayExists = await queryRunner.hasTable(
      'transacoes_gateway_pagamento',
    );
    const configuracoesGatewayExists = await queryRunner.hasTable(
    ...
    if (historicoPlanosExists) {
      await queryRunner.query(
        `ALTER TABLE "historico_planos" DROP CONSTRAINT IF EXISTS "historico_planos_empresa_id_fkey"`,
      );
    }

    if (modulosEmpresasExists) {
      await queryRunner.query(
        `ALTER TABLE "modulos_empresas" DROP CONSTRAINT IF EXISTS "modulos_empresas_empresa_id_fkey"`,
      );
    }

    await queryRunner.query(`DROP INDEX IF EXISTS "public"."idx_historico_planos_empresa_id"`);
    await queryRunner.query(
      `DROP INDEX IF EXISTS "public"."idx_historico_planos_data_alteracao"`,
    );
    await queryRunner.query(`DROP INDEX IF EXISTS "public"."idx_modulos_empresas_modulo"`);
    await queryRunner.query(`DROP INDEX IF EXISTS "public"."idx_modulos_empresas_ativo"`);
    await queryRunner.query(`DROP INDEX IF EXISTS "public"."idx_modulos_empresas_empresa_id"`);
    await queryRunner.query(`
            DO $$
            BEGIN
    ...
    // ⚡ MULTI-TENANT: Habilitar RLS + política idempotente (mesmo padrão já usado em outras migrations)
    await queryRunner.query(`ALTER TABLE "user_activities" ENABLE ROW LEVEL SECURITY;`);
    await queryRunner.query(`
      DO $$
      BEGIN
        IF NOT EXISTS (
          ...
          WHERE schemaname = 'public'
            ...
            AND policyname = 'tenant_isolation_user_activities'
        ) THEN
    ...
    if (faturasExists) {
      await queryRunner.query(`ALTER TABLE "faturas" ADD COLUMN IF NOT EXISTS "empresa_id" uuid`);
          ...
          await queryRunner.query(`
            ...
            BEGIN
              IF NOT EXISTS (
                ...
                AND to_regclass('public.empresas') IS NOT NULL THEN
            ...
            BEGIN
                IF NOT EXISTS (
                    ...
                    IF to_regclass('public.contas_pagar') IS NOT NULL
                      AND to_regclass('public.fornecedores') IS NOT NULL THEN
            ...
            BEGIN
                IF NOT EXISTS (
                    ...
                    WHERE constraint_name = 'FK_e6bd2112e7b77236fa767be0f0d'
                      ...
                      AND table_schema = 'public'
                ) AND to_regclass('public.historico_planos') IS NOT NULL
                  AND to_regclass('public.empresas') IS NOT NULL THEN
            ...
            BEGIN
                IF NOT EXISTS (
                    ...
                    WHERE constraint_name = 'FK_8ae75d87142f998d9083fdadd76'
                      ...
                      AND table_schema = 'public'
                ) AND to_regclass('public.modulos_empresas') IS NOT NULL
                  AND to_regclass('public.empresas') IS NOT NULL THEN
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE INDEX "idx_modulos_empresas_empresa_id" ON "modulos_empresas" ("empresa_id") `,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_modulos_empresas_ativo" ON "modulos_empresas" ("ativo") `,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_modulos_empresas_modulo" ON "modulos_empresas" ("modulo") `,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_historico_planos_data_alteracao" ON "historico_planos" ("data_alteracao") `,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_historico_planos_empresa_id" ON "historico_planos" ("empresa_id") `,
    );
    await queryRunner.query(
      `ALTER TABLE "modulos_empresas" ADD CONSTRAINT "modulos_empresas_empresa_id_fkey" FOREIGN KEY ("empresa_id") REFERENCES "empresas"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "historico_planos" ADD CONSTRAINT "historico_planos_empresa_id_fkey" FOREIGN KEY ("empresa_id") REFERENCES "empresas"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "faturas" ADD CONSTRAINT "FK_644bf709a39af5c760b98e56a06" FOREIGN KEY ("empresa_id") REFERENCES "empresas"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
