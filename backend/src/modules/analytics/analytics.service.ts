...
export class AnalyticsService {
  ...
  constructor() {} // @InjectRepository(Fatura) private faturaRepo: Repository<Fatura>, // @InjectRepository(Contrato) private contratoRepo: Repository<Contrato>, // @InjectRepository(Proposta) private propostaRepo: Repository<Proposta>, // Injete aqui os repositórios necessários quando tiver as entidades criadas
  ...
  async getFunilConversao(params: AnalyticsQueryParams) {
    try {
      ...
      return {
        ...
        gargalos: [
          {
            etapa: 'Envio → Aprovação',
            taxa_conversao: 47.4,
            melhoria_sugerida: 'Melhorar qualificação inicial',
          },
          {
            etapa: 'Aprovação → Assinatura',
            taxa_conversao: 84.4,
            melhoria_sugerida: 'Agilizar processo de contrato',
          },
        ],
      };
    } catch (error) {
  ...
  async exportarRelatorio(params: AnalyticsQueryParams): Promise<Buffer> {
    try {
      ...
      funilSheet.addRow([
        'Propostas Enviadas',
        data.funil.propostas_enviadas,
        `${data.funil.conversao_por_etapa.criada_para_enviada}%`,
      ]);
      funilSheet.addRow([
        'Propostas Aprovadas',
        data.funil.propostas_aprovadas,
        `${data.funil.conversao_por_etapa.enviada_para_aprovada}%`,
      ]);
      funilSheet.addRow([
        'Contratos Assinados',
        data.funil.contratos_assinados,
        `${data.funil.conversao_por_etapa.aprovada_para_assinada}%`,
      ]);
      funilSheet.addRow([
        'Faturas Pagas',
        data.funil.faturas_pagas,
        `${data.funil.conversao_por_etapa.assinada_para_paga}%`,
      ]);
