...
type LegacyTransitionMode = 'legacy' | 'dual' | 'canary' | 'guardian_only';

@Injectable()
export class LegacyAdminTransitionGuard implements CanActivate {
  private static readonly READ_ONLY_ALLOWED_METHODS = new Set(['GET', 'HEAD', 'OPTIONS']);

  private readonly mode: LegacyTransitionMode;
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (this.shouldForceGuardianForCriticalLegacyRoute(method, currentRoute)) {
      throw new GoneException({
        ...
        message:
          'Endpoint critico legado desativado para escrita. Execute esta operacao no namespace /guardian/*.',
        route: currentRoute,
        method,
        mode: this.mode,
    ...
    response?.setHeader?.(
      ...
      migrateToGuardian ? 'guardian' : 'legacy-admin',
    );
    ...
    throw new GoneException({
      ...
      message: 'Endpoint legado admin desativado para este ator. Use o namespace /guardian/*.',
      route: currentRoute,
      mode: this.mode,
      guardianBasePath: '/guardian',
    });
  ...
  private shouldForceGuardianForCriticalLegacyRoute(method: string, route: string): boolean {
    ...
    if (LegacyAdminTransitionGuard.READ_ONLY_ALLOWED_METHODS.has(method)) {
      return false;
    }
  ...
  private ensureWriteAllowed(method: string, route: string): void {
    ...
    throw new MethodNotAllowedException({
      ...
      message:
        'Backoffice legado em modo somente leitura. Execute operacoes de escrita no namespace /guardian/*.',
      route,
      method,
      mode: this.mode,
      guardianBasePath: '/guardian',
