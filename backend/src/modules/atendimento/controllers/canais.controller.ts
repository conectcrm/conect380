...
export class CanaisController {
  ...
  async listar(@Req() req) {
    ...
    try {
      ...
      if (!empresaId) {
        console.warn('⚠️ [CanaisController] empresaId ausente no token do usuário. Abortando listagem.');
        return {
          success: false,
      ...
      console.log('🔍 [CanaisController] Canais encontrados:', canais.length);
      ...
      console.log('🤖 [CanaisController] Configurações de IA encontradas:', configsIA.length);

      // Converter configurações de IA para formato de canais
      const canaisIA = configsIA.map(config => ({
        id: config.id,
        tipo: config.tipo,
        nome: `${config.tipo.toUpperCase()} Principal`,
        ativo: config.ativo,
        empresaId: config.empresaId,
        configuracao: {
          credenciais: config.credenciais
        },
        createdAt: config.criadoEm,
        updatedAt: config.atualizadoEm,
      }));

      console.log('🔍 [CanaisController] Canais IA convertidos:', canaisIA.length);

      // Combinar canais normais com IA
      const todosCanais = [...canais, ...canaisIA];
  ...
  async criar(@Req() req, @Body() dto: any) {
    ...
    if (!empresaId) {
      console.warn('⚠️ [CanaisController] empresaId ausente no token do usuário. Abortando criação.');
      return {
        success: false,
        message: 'empresaId ausente no token do usuário',
      };
    ...
    if (tipo === 'openai' || tipo === 'anthropic') {
      ...
      try {
        ...
        console.log('📝 [CanaisController] Credenciais recebidas:', JSON.stringify(credenciais, null, 2));
        ...
        if (tipo === 'openai') {
          ...
          credenciaisFormatadas.model = credenciais.model || credenciais.openai_model || 'gpt-4o-mini';
          credenciaisFormatadas.max_tokens = credenciais.max_tokens || credenciais.openai_max_tokens || 2000;
          credenciaisFormatadas.temperature = credenciais.temperature || credenciais.openai_temperature || 0.7;
          ...
          credenciaisFormatadas.model = credenciais.model || credenciais.anthropic_model || 'claude-3-5-sonnet-20241022';
          credenciaisFormatadas.max_tokens = credenciais.max_tokens || credenciais.anthropic_max_tokens || 2000;
          credenciaisFormatadas.auto_responder = credenciais.auto_responder || false;
        }

        console.log('📝 [CanaisController] Credenciais formatadas:', JSON.stringify(credenciaisFormatadas, null, 2));
    ...
    console.log('📝 [CanaisController] Configuracao recebida:', JSON.stringify(dto.configuracao, null, 2));
    ...
    if (tipo === 'whatsapp' || tipo?.toLowerCase() === 'whatsapp') {
      ...
      configuracaoFinal = {
        credenciais: {
          ...
          whatsapp_business_account_id: credenciaisRecebidas.whatsapp_business_account_id || credenciaisRecebidas.business_account_id,
          whatsapp_webhook_verify_token: credenciaisRecebidas.whatsapp_webhook_verify_token || credenciaisRecebidas.webhook_verify_token || 'conectcrm_webhook_token_123',
        }
      };

      console.log('✅ [CanaisController] WhatsApp - Configuração normalizada:', JSON.stringify(configuracaoFinal, null, 2));
    }
  ...
  async atualizar(
    ...
    if (dto.configuracao !== undefined) {
      ...
      if (canal.tipo === 'whatsapp' || canal.tipo?.toLowerCase() === 'whatsapp') {
        ...
        console.log('🔄 [CanaisController] Merge WhatsApp - Credenciais antes:', JSON.stringify(credenciaisExistentes, null, 2));
        console.log('🔄 [CanaisController] Merge WhatsApp - Credenciais novas:', JSON.stringify(novasCredenciais, null, 2));
        console.log('✅ [CanaisController] Merge WhatsApp - Credenciais mescladas:', JSON.stringify(credenciaisMerged, null, 2));
    ...
    if (tipoCanal === 'whatsapp' || tipoCanal === 'whatsapp_business_api') {
      console.log('🔄 [CanaisController] Atualizando atendimento_integracoes_config para WhatsApp...');
      ...
      try {
        ...
        console.log('📝 [CanaisController] Credenciais a salvar:', JSON.stringify(credenciais, null, 2));
        ...
        if (integracao) {
          ...
          console.log('✅ [CanaisController] Credenciais JSONB:', JSON.stringify(integracao.credenciais, null, 2));
          console.log('✅ [CanaisController] Token coluna:', integracao.whatsappApiToken?.substring(0, 20) + '...');
