...
@Injectable()
export class CoreAdminCriticalAuditInterceptor implements NestInterceptor {
  private readonly criticalMethods = new Set<string>(['POST', 'PUT', 'PATCH', 'DELETE']);

  constructor(private readonly coreAdminCriticalAuditService: CoreAdminCriticalAuditService) {}
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    return next.handle().pipe(
      tap((data) => {
        void this.coreAdminCriticalAuditService.record({
          ...basePayload,
  ...
  private normalizeRoute(rawRoute: string): string {
    ...
    return rawRoute.split('?')[0] || '/core-admin';
  }
