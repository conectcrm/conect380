...
@Injectable()
export class CoreAdminBffAuditInterceptor implements NestInterceptor {
  private readonly logger = new Logger(CoreAdminBffAuditInterceptor.name);
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    const registerAudit = (statusCode: number, outcome: 'success' | 'error', error?: unknown) => {
      ...
      const descricao = `Gateway core-admin ${method} ${route} (${statusCode})`;
      void this.userActivitiesService
        ...
        .catch((auditError) => {
          this.logger.warn(
            `Falha ao registrar auditoria do core-admin-bff para ${method} ${route}: ${
  ...
  private normalizeRoute(rawRoute: string): string {
    if (!rawRoute) {
      return '/core-admin/bff';
    }
    return rawRoute.split('?')[0] || '/core-admin/bff';
  }
