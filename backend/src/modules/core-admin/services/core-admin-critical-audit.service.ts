...
export type CoreAdminCriticalAuditInput = {
  actorUserId: string;
...
export type CoreAdminCriticalAuditListFilters = {
  page?: number;
  limit?: number;
  outcome?: string;
  method?: string;
...
export type CoreAdminCriticalAuditListResult = {
  data: CoreAdminCriticalAudit[];
  meta: {
    total: number;
    page: number;
    limit: number;
...
@Injectable()
export class CoreAdminCriticalAuditService {
  private readonly logger = new Logger(CoreAdminCriticalAuditService.name);
  ...
  constructor(
    @InjectRepository(CoreAdminCriticalAudit)
    private readonly coreAdminCriticalAuditRepository: Repository<CoreAdminCriticalAudit>,
  ) {}

  async record(input: CoreAdminCriticalAuditInput): Promise<void> {
    try {
      await this.coreAdminCriticalAuditRepository.insert({
      ...
      this.logger.warn(
        `Falha ao registrar auditoria critica core-admin: ${
  ...
  async list(
    filters: CoreAdminCriticalAuditListFilters = {},
  ): Promise<CoreAdminCriticalAuditListResult> {
    const page = this.clamp(filters.page, 1, 100000, 1);
    const limit = this.clamp(filters.limit, 1, 200, 20);

    const query = this.coreAdminCriticalAuditRepository
  ...
  async listForExport(filters: CoreAdminCriticalAuditListFilters = {}): Promise<CoreAdminCriticalAudit[]> {
