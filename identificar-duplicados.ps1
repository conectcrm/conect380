...
function Find-SimilarFiles {
    ...
    foreach ($file in $files) {
        $hash = Get-FileHash-Content $file
        if ($hash) {
            if (!$fileGroups[$hash]) {
                $fileGroups[$hash] = @()
            }
    ...
    if ($similarNames) {
        ...
        foreach ($group in $similarNames) {
            ...
            foreach ($file in $group) {
                ...
                $modified = (Get-Item $file).LastWriteTime.ToString("dd/MM/yyyy HH:mm")
                Write-Host "        - $file ($size bytes, $modified)" -ForegroundColor Yellow
            }
        }
    }
}
