  ...
  return (
    <div className="pointer-events-none fixed bottom-6 right-6 z-[60] flex flex-col gap-3 max-w-full">
      {notifications.map((notification) => {
        ...
        const elapsed = formatDistanceToNow(notification.createdAt, {
          addSuffix: true,
          locale: ptBR,
        });
        ...
        return (
          <div
            ...
            className={`pointer-events-auto relative w-80 max-w-[calc(100vw-3rem)] rounded-2xl border border-gray-200 bg-white shadow-lg transition-all ${
              clickable ? 'cursor-pointer hover:-translate-y-0.5 hover:shadow-xl' : ''
            }`}
          >
            <span
              className={`absolute inset-y-0 left-0 w-1 rounded-l-2xl ${accentClass}`}
              aria-hidden="true"
            />
            ...
            <div className="flex gap-3 p-4 pr-5">
              <div className="relative flex-shrink-0">
                ...
                {CanalIcon && (
                  <div className="absolute -bottom-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-white shadow">
                    <CanalIcon
                      className={`h-3 w-3 ${notification.type === 'novo-ticket' ? 'text-amber-500' : 'text-blue-500'}`}
                    />
                  </div>
                )}
              </div>
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-3">
                  <span
                    className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide ${badgeClass}`}
                  >
                    {notification.type === 'novo-ticket' ? (
                  ...
                  <span className="flex-shrink-0 text-[11px] text-gray-400">{elapsed}</span>
                </div>

                <h4 className="mt-2 text-sm font-semibold text-gray-900">{notification.title}</h4>
