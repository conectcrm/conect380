...
interface Cliente {
  ...
  telefone: string;
  documento: string;
  endereco?: string; // Tornando opcional para compatibilidade
  cidade?: string; // Tornando opcional para compatibilidade
  estado?: string; // Tornando opcional para compatibilidade
  cep?: string; // Tornando opcional para compatibilidade
  observacoes?: string;
  ativo?: boolean;
  createdAt?: string;
  updatedAt?: string;
        ...
        <div className="flex items-center space-x-2">
          ...
          {onNewCliente && (
            <button
              ...
              className="flex items-center justify-center w-10 h-10 bg-teal-600 hover:bg-teal-700 text-white rounded-lg transition-colors"
        ...
        {isLoading ? (
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-xs text-gray-500 mb-4">
              {searchTerm
                ? 'Tente ajustar sua pesquisa ou use o botão + para cadastrar um novo cliente.'
                : 'Use o botão + para cadastrar seu primeiro cliente.'}
            </p>
          </div>
        )}
      </div>
