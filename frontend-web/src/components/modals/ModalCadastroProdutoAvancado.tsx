...
import {
  useSegmentoConfig,
  type SegmentoConfig as SegmentoConfigType,
  SEGMENTOS_CONFIGURACAO,
} from '../../hooks/useSegmentoConfig';
...
export const ModalCadastroProdutoAvancado: React.FC<ModalCadastroProdutoAvancadoProps> = ({
  ...
  const [tabAtiva, setTabAtiva] = useState<
    'basico' | 'avancado' | 'modulos' | 'licencas' | 'personalizados'
  >('basico');
  ...
  const {
    fields: licencasFields,
    append: appendLicenca,
    remove: removeLicenca,
  } = useFieldArray({
    control,
    name: 'licencas',
  });
  ...
  const configTipoProduto = segmentoConfig.tiposProduto.find(
    (t) => t.value === tipoProdutoSelecionado,
  );
          ...
          <nav className="flex space-x-8">
            {[
              ...
              { id: 'avancado', label: 'Configurações', icon: Settings },
              {
                ...
                label: 'Módulos',
                icon: Layers,
                show: ['plano', 'combo'].includes(tipoProdutoSelecionado),
              },
              {
                id: 'licencas',
                label: 'Licenças',
                icon: Shield,
                show: ['plano', 'licenca', 'combo'].includes(tipoProdutoSelecionado),
              },
            ...
            <AdaptiveColumns minWidth={isMobile ? 280 : isTablet ? 320 : 350}>
              ...
              <ResponsiveCard>
                ...
                <div className="space-y-4">
                  <FormField label="Nome do Produto" error={errors.nome?.message} required>
                    <BaseInput
                      {...register('nome')}
                  ...
                  <FormField label="Código/SKU" error={errors.codigo?.message} required>
                    <BaseInput
                      {...register('codigo')}
                  ...
                  <FormField label="Tipo de Produto" error={errors.tipoProduto?.message} required>
                    <BaseSelect
              ...
              <ResponsiveCard>
                ...
                <div className="space-y-4">
                  ...
                  {tipoPreco === 'variavel' && (
                    <>
                      <FormField label="Preço Mínimo" error={errors.precoMinimo?.message}>
                        <BaseInput
                      ...
                      <FormField label="Preço Máximo" error={errors.precoMaximo?.message}>
                        <BaseInput
                ...
                {configTipoProduto?.campos && configTipoProduto.campos.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {configTipoProduto.campos.map((campo, index) => (
                      ...
                      >
                        {campo.tipo === 'texto' && (
                          <BaseInput placeholder={campo.placeholder} className="w-full" />
                        )}
                        {campo.tipo === 'numero' && (
                          <BaseInput
                            type="number"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                          <FormField label="Módulo">
                            <BaseSelect
                              ...
                              options={
                                configTipoProduto?.modulosDisponiveis?.map((mod) => ({
                                  value: mod,
                                  label: mod,
                                })) || []
                              }
                          ...
                          <FormField label="Quantidade">
                            <BaseInput
                              type="number"
                              min="1"
                              {...register(`modulos.${index}.quantidade` as const, {
                                valueAsNumber: true,
                              })}
                              className="w-full"
                            />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                          <FormField label="Aplicativo">
                            <BaseSelect
                              ...
                              options={
                                configTipoProduto?.licencasDisponiveis?.map((lic) => ({
                                  value: lic,
                                  label: lic,
                                })) || []
                              }
                          ...
                          <FormField label="Quantidade">
                            <BaseInput
                              type="number"
                              min="1"
                              {...register(`licencas.${index}.quantidade` as const, {
                                valueAsNumber: true,
                              })}
                              className="w-full"
                            />
