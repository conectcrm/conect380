      ...
      <div className="px-4 py-3">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            <div className={`w-6 h-6 ${currentProfileData?.avatarBg} rounded-full flex items-center justify-center`}>
              <span className="text-white text-xs font-bold">
                {currentProfileData?.label.charAt(0)}
              </span>
            </div>
            <span className="text-xs text-gray-600 font-medium">
              {currentProfileData?.label}
            </span>
          </div>
        </div>
        ...
        <div className="space-y-1 max-h-48 overflow-y-auto">
          {PERFIS_DISPONIVEIS.map((perfil) => (
            <button
              ...
              className={`w-full flex items-center gap-3 p-3 rounded-lg text-left transition-all duration-200 hover:bg-gray-50 ${currentProfile === perfil.value
                  ? 'bg-[#159A9C]/5 border-l-4 border-l-[#159A9C] shadow-sm'
                  : 'hover:shadow-sm'
                }`}
            >
              <div className={`w-8 h-8 ${perfil.avatarBg} rounded-lg flex items-center justify-center transition-all duration-200 ${currentProfile === perfil.value ? 'shadow-md' : 'shadow-sm'
                }`}>
                <span className="text-white text-xs font-bold">
              ...
              <div className="flex-1 min-w-0">
                <div className={`text-sm font-medium ${currentProfile === perfil.value ? 'text-[#159A9C]' : 'text-gray-900'
                  }`}>
                  {perfil.label}
                </div>
