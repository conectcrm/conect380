...
export interface RespostaIA {
  resposta: string;
  confianca: number;
   // Detectar saudações e cumprimentos - EXPANDIDO
  private detectarSaudacao(mensagem: string): boolean {
    ...
    const msgLower = this.limparTexto(mensagem);
    return saudacoes.some(saudacao => 
      msgLower.includes(saudacao) || 
      msgLower.startsWith(saudacao) ||
      new RegExp(`\\b${saudacao}\\b`).test(msgLower)
    );
  }

  // Detectar sentimento na mensagem - NOVO
  private detectarSentimento(mensagem: string): 'positivo' | 'negativo' | 'neutro' {
    const msgLimpa = this.limparTexto(mensagem);
  ...
  // Extrair entidades da mensagem - NOVO
  private extrairEntidades(mensagem: string): { tipo: string; valor: string }[] {
...
class IAService {
  ...
  private inicializarBaseConhecimento() {
    // === MÓDULO DE DIÁLOGOS CONVERSACIONAIS ===
    this.baseConhecimento.set('dialogos', {
      categoria: 'conversacao',
      intents: [
        // Saudações básicas
        'olá', 'oi', 'hey', 'e aí', 'salve', 'bom dia', 'boa tarde', 'boa noite',
        'tudo bem', 'como vai', 'beleza', 'hello', 'hi',
        
        // Despedidas
        'tchau', 'até logo', 'bye', 'falou', 'até mais', 'obrigado', 'valeu',
        'até a próxima', 'foi um prazer', 'muito obrigado',
        
        // Cortesias
        'por favor', 'com licença', 'desculpa', 'perdão', 'obrigado', 'de nada',
        'não há de quê', 'imagina', 'disponha',
        
        // Expressões de satisfação/insatisfação
        'muito bom', 'excelente', 'perfeito', 'ótimo', 'legal', 'bacana',
        'não gostei', 'ruim', 'péssimo', 'complicado', 'difícil',
        
        // Confirmações/Negações
        'sim', 'não', 'claro', 'certeza', 'pode ser', 'talvez', 'quem sabe',
        'com certeza', 'óbvio', 'jamais', 'nunca', 'sempre',
