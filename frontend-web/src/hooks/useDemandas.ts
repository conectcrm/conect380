...
import { clientesService, Demanda, CreateDemandaDto, UpdateDemandaDto } from '../services/clientesService';
...
export const useDemandas = () => {
   ...
   */
  const carregarDemandas = useCallback(async (params: {
    clienteId?: string;
    telefone?: string;
    ticketId?: string;
    empresaId?: string;
    ...
    try {
      ...
      if (params.clienteId) {
        resultado = await clientesService.listarDemandasCliente(params.clienteId, params.empresaId);
      } else if (params.telefone) {
        resultado = await clientesService.listarDemandasPorTelefone(params.telefone, params.empresaId);
   ...
   */
  const carregarDemandaPorStatus = useCallback(async (
   ...
   * Criar nova demanda
   */
  const criarDemanda = useCallback(async (demandaData: CreateDemandaDto): Promise<Demanda | null> => {
    try {
      setLoading(true);
      setError(null);
   ...
   */
  const atualizarDemanda = useCallback(async (
    demandaId: string,
    demandaData: UpdateDemandaDto
    ...
    try {
      ...
      setDemandas((prev) =>
        prev.map((d) => (d.id === demandaId ? demandaAtualizada : d))
      );
   ...
   */
  const atribuirResponsavel = useCallback(async (
    demandaId: string,
    ...
    try {
      ...
      const demandaAtualizada = await clientesService.atribuirResponsavel(demandaId, responsavelId);
      setDemandas((prev) =>
        prev.map((d) => (d.id === demandaId ? demandaAtualizada : d))
      );
