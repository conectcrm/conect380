...
export const useNotificacao = (): UseNotificacaoReturn => {
  ...
  const mostrarSucesso = useCallback(
    (titulo: string, mensagem: string, autoClose: boolean = true) => {
  ...
  const mostrarErro = useCallback(
    (titulo: string, mensagem: string, autoClose: boolean = false) => {
  ...
  const sucesso = {
    ...
    pagamentoRegistrado: (valor?: number) => {
      const valorFormatado = valor
        ? `de R$ ${valor.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`
        : '';
      notificacao.mostrarSucesso(
  ...
  const erro = {
    operacaoFalhou: (operacao: string, detalhes?: string) => {
      notificacao.mostrarErro(
        'Operação Falhou',
        detalhes
          ? `Erro ao ${operacao}: ${detalhes}`
          : `Erro ao executar ${operacao}. Tente novamente.`,
      );
    },
